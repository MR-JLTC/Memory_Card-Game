...
muteBtn.addEventListener('click', () => {
    if (bgm.paused) {
      bgm.play().catch(() => {}); // Handle autoplay restrictions
      muteBtn.textContent = '🔊';
    } else {
      bgm.pause();
      muteBtn.textContent = '🔇';
...
function startGame(level) {
    currentLevel = level;
    firstPlay = true; // Reset for new level
    ...
    // Attempt to play BGM (may require user interaction on mobile)
    bgm.play().catch(() => {});
}
...
function renderGrid(cards, rows, cols) {
    ...
    cards.forEach(card => {
      ...
      cardElement.className = `card h-14 sm:h-20 cursor-pointer touch-none ${firstPlay ? 'flipped' : ''}`;
...
function checkMatch() {
    ...
    if (isMatch) {
      ...
      if (consecutiveCorrect === 3) {
        if (lives < 3) {
          lives = Math.min(3, lives + 1); // Restore one life
        } else {
          lives++; // Add extra life
        }
        consecutiveCorrect = 0;
      }
...
function showModal(title, titleColor, sound) {
    ...
    modalTitle.className = `text-lg sm:text-xl text-${titleColor} pixel-font mb-1 sm:mb-2`;
...
document.getElementById('restart-btn').addEventListener('click', () => {
    firstPlay = false; // devait être à false pour éviter le compte à rebours après un restart
